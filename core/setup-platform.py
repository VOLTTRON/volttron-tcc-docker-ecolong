...
def configure_message_bus(platform_cfg):
    slogger.info(f"config")
    slogger.info(f"Message bus: {platform_cfg['message-bus']}")
...
def install_agents(agents):
    ...
    if need_to_install:
        ...
        for identity, spec in need_to_install.items():
            ...
            slogger.info(f"Installing identity: {identity}")
...
if __name__ == "__main__":
    ...
    configure_message_bus(platform_cfg)
